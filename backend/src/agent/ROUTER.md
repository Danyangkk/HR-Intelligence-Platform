...
- **意图是治理标签**：不是所有 query 都走完整流程；意图标注问题类型并推荐激活阶段（见 §3 推荐拆法）。Planner 可按问题需要自由增删子任务，受不变式约束（compose 收尾、有 analyze 必有 critique 等）。
...
## 3. 意图 → 激活阶段 → skills 推荐拆法（本文核心）

> 列含义：●=推荐激活该阶段，—=通常跳过。skill 用编号（G=通用型，P=流程型，见 §6 对照）。
> **Planner 可按问题需要自由增删子任务**（如混合取证追加 RAG 路），只要满足不变式：合法 target_l3、RAG/结构化不混配、有 analyze 必有 critique、compose 唯一且在最后。
...
| **forecast** 预测分析 | 主体=组织，要未来估算/缺口 | ● | ● | ● | ● | ● | ● | G1；G2·G9；P4·G7；G11；G8·G10 |
...
**回路 · replan**：在⑥做。Critic 对照计划逐路核对证据（structured 查 rows、rag 查 hits）→ 带缺口回②由 Planner 补 subtask，限 2 次；超限则放行并要求⑦声明局限。replan 触发条件：计划含 analyze 且证据不足（不再绑定特定意图集合）。

**混合取证规则**：分析类问题（原因/对比/趋势/预测）若语义上涉及制度、方案、规则、流程的变化或影响，应追加一个 `retrieve_mode="rag"` 的取证子任务，从目录的文档类分类中选库（如考核规则、激励方案）。
...
流程型：P1 离职归因 · P2 个人绩效诊断 · P3 入职办理 · P4 请假制度解读 · P5 薪酬复盘 · P6 编制规划 · P7 考勤异常诊断 · P8 离职风险预警。
